...
const CreatePost = ({ open, setOpen }) => {
    ...
    const handlePost = () => {
        alert(
            `Post created in "${category}" with tags [${tags.join(", ")}]: ${postText}`
        );
        resetForm();
        setOpen(false);
    };
                        ...
                        <>
                            ...
                            {availableTags.length > 0 && (
                                <div className="px-6 py-3 flex flex-wrap gap-2">
                                    {availableTags.map((tag) => (
                                        <label
                                            key={tag}
                                            className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm cursor-pointer transition border ${tags.includes(tag)
                            ...
                            <div className="px-6 py-4 border-t border-gray-800">
                                <button
                                    ...
                                    disabled={!imagePreview || tags.length === 0}
                                    onClick={handlePost}
                                >
                                    Post
                                </button>
