...
const Post = ({ post }) => {
  ...
  return (
    <div className='select-none my-8 w-full max-w-sm mx-auto'>
      ...
      {/* Images (UI SAME, MULTI SUPPORT ADDED) */}
      {Array.isArray(post.image) && post.image.length > 0 && (
        <>
          <div
            className='rounded-sm my-2 w-full aspect-square object-cover overflow-x-auto flex snap-x snap-mandatory no-scrollbar'
          ...
          >
            {post.image.map((img, index) => (
              <img
                key={index}
                src={img}
                className='w-full min-w-full aspect-square object-cover snap-center'
                alt="post_img"
              />
            ))}
          </div>
